...
#from Input_and_Constants _SIGMAS,_Z0
...
def select_ieo_by_approx_lnlike_dist_marginalized_cupy(
        dh_ieo, 
        hh_ieo,
        log_weights_i, 
        log_weights_e,
...
__global__ void lnl_approx_array_create(const float* pdh_ieo
                        , const float* phh_ieo
                        , const float* plog_weights_i
                        , const float* plog_weights_e                        
                        , const int i0
                        , const int i1
                        , const int i2
                        ,unsigned int N
                        ...
                        , float* plnl_approx )
{
    ...
    if (tid < N)
    {
        float val_h_norm = sqrt(phh_ieo[tid] ) ;  
        float z = pdh_ieo [tid]/ val_h_norm;
...
def select_ieo_by_approx_lnlike_dist_marginalized_new(
    ...
    lnl_approx = cp.zeros((N,), dtype = cp.float32)
...
def calculate_lnlike_and_evidence_cupy(n_phi,m_arr_, m_inds,mprime_inds,dh_weights_dmpb,h_impb,response_dpe, timeshift_dbe,                          
                                hh_weights_dmppb,
                                asd_drift_d, 
                                log_weights_i,                                
                                log_weights_e, 
                                ...
                                ,y_arr
                                ,table                             
                                ,approx_dist_marg_lnl_drop_threshold_: float = 20., 
    ...
    if bprofile:            
            t0 = time.time()
    approx_dist_marg_lnl_drop_threshold = cp.array(approx_dist_marg_lnl_drop_threshold_)

    arr = approx_dist_marg_lnl_drop_threshold.get()
    ...
    if bprofile:
         t12 = time.time()
    if TYPE_OF_PROGRAM == 'STADART_CUPY':
        d_inds_i_k, d_inds_e_k, d_inds_o_k \
            = select_ieo_by_approx_lnlike_dist_marginalized_cupy(            
                d_dh_ieo, d_hh_ieo, d_log_weights_i, d_log_weights_e, 
                approx_dist_marg_lnl_drop_threshold)
    if TYPE_OF_PROGRAM == 'WITH_CUSTOMIZED_KERNELS_CUPY':
        d_inds_i_k, d_inds_e_k, d_inds_o_k \
    ...
    if frac_evidence_threshold:
        ...
        if bprofile:
             ...
             print('Time of copying data + interpolator creation                     = ', (t35 - t0) * 1e3 - arr_dt.sum(), ' (ms)')
             ...
             print('   Time of initial copying data to device + interpolator creation = ', arr_dt[0],' (ms)')
